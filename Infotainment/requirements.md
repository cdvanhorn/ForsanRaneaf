...
* The application will use raygui for the UI (https://github.com/raysan5/raygui) (MVP)
...
* The application will display detailed status (MVP)
* The application will have a settings interface
* The application will record vehicle history
* The application will run on a Raspberry Pi 5 CM with the Raspberry Pi touch display 2 1280x720 resolution (https://www.raspberrypi.com/products/touch-display-2/) with Raspbian or DietPi OS (MVP)
...
* The application will require 6 digit code entry before allowing contactors to close and emergency brake to release and vehicle to drive
  * Unless in shop mode in which case a code will not be required to close contactors or release/engage emergency brake
* The application will use a sqlite database for storage (history, settings, ...)
* There will be physical buttons for music control
